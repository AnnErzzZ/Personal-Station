...
// 组件需要先量出 SVG 的包围盒再播放，服务端渲染时没有 window。
...
const StrokeText = ({
  ...
  const fontSizeValue = toNumber(fontSize, DEFAULT_FONT_SIZE);
  const dash = Math.max(fontSizeValue * 7, 200);

  const fontStyle = useMemo(
    () => ({
      fontSize: toLength(fontSize),
      fontWeight,
      letterSpacing: toLength(letterSpacing),
    }),
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    measure();
    // 没有 webfont 时 document.fonts.ready 早已 resolve，再测一次只会得到
    // 亚像素差异，触发 box 更新并把动画从头重播；只在字体确实在加载时补测。
      ...
      >
        ...
        >
          {characters.map((char, index) => (
            // 首屏 HTML 直接给出未绘制状态，避免水合前先闪一版完整字形。
            <tspan
              data-stroke-char
              key={`s-${index}`}
        ...
        >
          {characters.map((char, index) => (
            <tspan data-fill-char key={`f-${index}`} opacity={0}>
              {char}
            </tspan>
          ))}
        </text>
