...
/**
 * GradientWaves — React Bits (reactbits.dev)，JavaScript + CSS 变体。
 * 上游源码逐字保留，仅两处适配本项目「页面背景层」的用法：
 *   1) 指针监听从 canvas 改到 window —— 本组件挂在 pointer-events:none
 *      的背景层里，文案 / 导航都压在它上面，canvas 收不到 pointermove；
 *      坐标仍按 canvas 自身矩形归一化，行为与上游一致。
 *   2) canvas pointer-events:none（见 GradientWaves.css），不挡正文点击。
 */
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    // 适配点：监听 window 而不是 canvas（本组件作为背景层，
    // 上方文案会挡住 canvas 的命中测试；坐标仍按 canvas 矩形归一化）。
