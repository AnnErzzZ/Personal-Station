...
function buildGlyphFromDOM(count, lines, opts) {
  ...
  wrappedLines.forEach((line, i) => {
    ...
    const baseline = useMetrics ? lineTop + fbAsc : lineTop + fallbackAscent;
...
function smooth(a, b, x) {
  const t = THREE2__namespace.MathUtils.clamp((x - a) / (b - a), 0, 1);
...
function GlyphPoints(props) {
  ...
  const windowElsRef = react.useRef(/* @__PURE__ */ new Map());
  ...
  const built = react.useMemo(() => {
    const geo = new THREE2__namespace.BufferGeometry();
    ...
    buffers.forEach((buf, i) => {
      geo.setAttribute(
        ...
        new THREE2__namespace.BufferAttribute(buf, 3)
      );
    });
    geo.setAttribute("aSeed", new THREE2__namespace.BufferAttribute(seed, 1));
    geo.setAttribute("aAccent", new THREE2__namespace.BufferAttribute(accent, 1));
    const first = buffers[0] ?? new Float32Array(count * 3);
    geo.setAttribute("position", new THREE2__namespace.BufferAttribute(first.slice(), 3));
    geo.computeBoundingSphere();
    return { geo, buffers, visW, vpW, vpH };
  }, [keyframes, count, colors.accentRatio, cameraFov, cameraZ, style.scatterPattern]);
  const vertexShader = react.useMemo(() => buildVertexShader(Math.max(n, 1)), [n]);
  ...
  const trackBaseScrollYRef = react.useRef(0);
  ...
  const isFirstResample = react.useRef(true);
  react.useEffect(() => {
  ...
  react.useEffect(() => {
    ...
    mat.blending = style.blend === "additive" ? THREE2__namespace.AdditiveBlending : THREE2__namespace.NormalBlending;
  ...
  fiber.useFrame((state) => {
    ...
    const raw = THREE2__namespace.MathUtils.clamp(getProgress(), 0, 1);
    stage.current = raw;
  ...
  return /* @__PURE__ */ jsxRuntime.jsx("points", { ref: pointsRef, geometry: built.geo, frustumCulled: false, children: /* @__PURE__ */ jsxRuntime.jsx(
    "shaderMaterial",
    {
      ...
      transparent: true,
      depthWrite: false,
      blending: style.blend === "additive" ? THREE2__namespace.AdditiveBlending : THREE2__namespace.NormalBlending,
      vertexShader,
...
function GlyphDust(props) {
  ...
  const resolvedColors = react.useMemo(
    () => ({
      ink: new THREE2__namespace.Color(colors?.ink ?? DEFAULT_INK),
      accent: new THREE2__namespace.Color(colors?.accent ?? DEFAULT_ACCENT),
      accentRatio: colors?.accentRatio ?? DEFAULT_ACCENT_RATIO
    }),
    [colors?.ink, colors?.accent, colors?.accentRatio]
  );
  const bloomWarningShownRef = react.useRef(false);
  react.useEffect(() => {
  ...
  const scene = /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsxs(
      fiber.Canvas,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntime.jsx(ResumeOnUnpause, { paused }),
          ...
          resolvedStyle.bloom > 0 && !mobile && BloomComponent ? /* @__PURE__ */ jsxRuntime.jsx(BloomComponent, { strength: resolvedStyle.bloom }) : null
        ]
      }
    ),
