...
interface GlyphStyle {
    ...
    burst?: number;
    /**
     * 粒子ごとの透明度ばらつきの強さ 0..1。**既定はプリセット依存（default 0.55）**。
     * 飛散中の粒に α の個体差（0.45〜1.0）を与え、浮遊感・奥行き感を作る。
     * 整列・字形時は自動で 1（均一）に畳まれ、文字の可読性は変わらない。
     * 経緯: 2026-07-11 に一度却下され既定0化→スムーズ化完了後の 2026-07-12 に
     * 凜さん「美しくする提案はデフォルトで全部入れて」で既定オンに復帰。
     */
    alphaVar?: number;
    /**
     * 擬似被写界深度（DoF）の強さ 0..1。**既定はプリセット依存（default 0.5、minimal 0）**。
     * フォーカス面（テキストの z=0 平面）から奥・手前に離れた粒ほど、レンズの
     * ボケ円のように「大きく・柔らかく・薄く・わずかに脱彩度」になり、飛散雲に
     * 奥行きの層が生まれる。整列した文字はフォーカス面上にあるため影響ゼロ。
     * 経緯: 2026-07-11 に明滅として知覚され却下→既定0化→スムーズ化完了後の
     * 2026-07-12 に凜さん「美しくする提案はデフォルトで全部入れて」で既定オンに復帰。
     * 明滅が気になる場合は style で 0 を明示指定。
     */
    dof?: number;
    /**
     * stagger（到達タイミング分散）の空間相関度 0..1。**既定はプリセット依存（default 0.75）**。
     * 0 = 従来の粒子ごと独立ランダム（一様にほどける）。
     * 1 = 静的な simplex ノイズ場で出発順を決める＝近くの粒が連れ立って発ち、
     * 「風が撫でるようにテキストが塊（斑）単位で粒子にばらけていく」有機的な溶解になる
     * （Codrops Gommage 2026-01 等の名作と同じノイズマスク設計）。
     * `stagger` が 0 のときは波も無効（分散する出発順そのものが無いため）。
     * ノイズの参照点は区間の出発駅位置で固定＝区間中の進捗は厳密に単調で、
     * 収束点への到達位置は不変（DOM 整列・resolve のピクセル一致保証は壊れない）。
     * 2026-07-11 品質向上 Phase 2 で追加→一括既定オンを凜さんが却下→
     * 凜さん依頼「テキストが粒子にばらけていくように」で opt-in 再導入→
     * 2026-07-12 凜さん「美しくする提案はデフォルトで全部入れて」で既定オンに復帰。
     */
    wave?: number;
    /**
     * 光（selective bloom）の強さ 0..1。既定は `glow` プリセットのみ 0.6、他は 0。
     * きらめき粒・アクセント粒だけが「内側から光る」発光ポストプロセッシング。
     * 粒子の軌道・タイミング（拡散・収束の動き）には一切影響しない。
     *
     * 発光を描画するには `glyphdust/bloom` から BloomEffect を import し、
     * {@link GlyphDustProps.bloomComponent} に渡す。渡さなければ bloom 無しで描画継続。
     * 暗背景向け（明背景では発光はほぼ見えない）。モバイルは負荷対策で自動オフ。
     * fps への影響（-10〜20% 目安）があるため既定はオプトイン。
     * 2026-07-11 品質向上「光」の軸として追加（提案者: Claude、凜さん指示
     * 「収束拡散は元のまま提案を全て実装」）。
     */
    bloom?: number;
    /**
     * 【fork 追加】粒子の溶解 / 収束を**文字単位**でずらす強さ -1..1。既定 0（無効）。
     *
     * 粒⼦ごとに「その粒子が属する文字」の番号（0 = 先頭、1 = 末尾）を持たせ、
     * その値で到達タイミング（uStagger の offset）を置き換える。
     * - `0`   … 従来どおり（aSeed のランダム × uWave のノイズ斑）
     * - `1`   … 先頭の文字から順に溶け / 先頭の文字から順に組み上がる
     * - `-1`  … 末尾の文字から順（逆向き）
     * 0..1 の中間値は「文字順」と「従来のランダム / ノイズ」のブレンド。
     * 上の `stagger` が「ずらし幅」なので、`charOrder` だけでは動きは変わらない
     * （`stagger > 0` と併用する）。
     */
    charOrder?: number;
    /**
     * 【fork 追加】文字 advance を測るための canvas font 文字列。
     * 省略時は `domSelector` の要素の computed font、無ければ keyframe の `font` を使う。
     */
    charFont?: string;
}
/**
...
interface GlyphDustProps {
    ...
    style?: GlyphStyle;
    /**
     * `glyphdust/bloom` から BloomEffect を import して渡す。
     * なぜ明示指定にするかというと、渡さない利用者のビルドへ optional peer を
     * 混ぜないため。渡さなければ bloom は描画されない。
     */
    bloomComponent?: ComponentType<{
        strength: number;
    }>;
    /** 配色。 */
    colors?: GlyphColors;
    ...
    timing?: number[];
    /**
     * 粒子の出現（swap）をフェードインにする進捗幅 0..1。**既定 0（従来どおり瞬時切替）**。
     * 0 より大きいと、swap 点から `swapFade` 幅かけて粒子の不透明度が 0→1 に滑らかに
     * 立ち上がる。「実テキストがスッと消える→凝縮した粒子テキストがふわっと表れる→
     * 拡散が始まる」という段階的な導入を、瞬時スワップの段差なしに作れる。
     * `timing[0] > 0`（最初のキーフレームでの静止保持）と併用すると、粒子が
     * 完全に静止した字形のままフェードインし、その後に初めて動き出す。
     * 2026-07-11 追加（提案者: 凜さん「テキストが2段階くらいで消えるようになってる？
     * スッと消えて→凝縮したテキストの形→拡散、をスムーズに」）。
     */
    swapFade?: number;
    /**
     * 粒子が現れる進捗点 0..1 の上書き。**既定は内部値 `timing[1] * 0.15`（従来どおり）**。
     * `timing[0]`（静止保持の終わり＝拡散開始点）に合わせると、静止中は実 DOM テキスト
     * だけが見え、**粒子が動き出す瞬間に初めて**クロスフェードが起きる。静止した
     * 粒子テキストと実テキストを並べて見せないため、点描の質感差・サンプリング厚みの
     * 視差ズレが原理的に見えなくなる（収束側の「動きが終わる瞬間に実テキストへ解決」の
     * 正確な鏡像）。2026-07-12 追加（提案者: 凜さん「テキストと粒子のズレをなくせばいい」）。
     */
    swapAt?: number;
...
export { type AutoplayDriverConfig, type DriverConfig, type GlyphCamera, type GlyphColors, type GlyphCount, GlyphDust, type GlyphDustProps, type GlyphPreset, type GlyphStyle, type Keyframe, type ManualDriverConfig, type ScatterKeyframe, type ScrollDriverConfig, type ShapeKeyframe, type TextKeyframe, type TextSegment };
